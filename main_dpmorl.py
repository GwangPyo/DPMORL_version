...
ENV_ID_TO_NAME = {
    ...
    "fishwood-v0": "FishWood",
    "four-room-v0": "FourRoom",
    "fruit-tree-v0": "FruitTree",
    "mo-highway-v0": "Highway",
    "mo-lunar-lander-v2": "LunarLander",
    "mo-halfcheetah-v5": "HalfCheetah",
    "mo-hopper-v5": "Hopper",
...
def build_test_env(env_id, *args, **kwargs):
    return mo_gymnasium.make(env_id, *args, **kwargs)
...
<<<<<<< HEAD
            if self.env_key in self.normalization_data.keys():
                utility_function.min_val = self.normalization_data[self.env_key]['min'][0][self.reward_dim_indices] / 2 # divide in 2, for max epi
...
>>>>>>> 4843851 (version consistency)
        ...
        data = evaluate(learned_policies, self.test_env, self.num_eval, self.reward_dim)
        os.makedirs('results', exist_ok=True)
        os.makedirs(f'results/{self.env_id}', exist_ok=True)
        data.to_csv(f'results/{self.env_id}/DPMORL_{self.seed}.csv')

    def __train_env_builder(self, ):

        def _init():
            env = mo_gymnasium.make(self.env_id,
